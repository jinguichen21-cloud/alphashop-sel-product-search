...
# 环境配置
API_ENDPOINTS = {
    "prod": "https://api.alphashop.cn/ai.sel.global1688.productSearchApi/1.0",
    "pre": "https://pre-api.alphashop.cn/ai.sel.global1688.productSearchApi/1.0",  # 预发环境
    "daily": "https://daily-api.alphashop.cn/ai.sel.global1688.productSearchApi/1.0"  # 日常环境
}

# 默认使用生产环境
DEFAULT_ENV = "prod"
API_ENDPOINT = API_ENDPOINTS[DEFAULT_ENV]
...
def call_api(request_data, user_id, access_key=None, secret_key=None, env="prod"):
    """调用 AlphaShop HTTP API"""
    endpoint = API_ENDPOINTS.get(env, API_ENDPOINTS["prod"])
...
示例:
  ...
  python3 scripts/search.py --keyword "phone" --platform amazon --region US \\
    ...
    parser.add_argument("--env", choices=["prod", "pre", "daily"], default="prod", help="API环境（prod=生产, pre=预发, daily=日常，默认prod）")
    ...
    # 调用接口
    result = call_api(request_data, args.user_id, args.app_key, args.app_secret, args.env)
